...
if(isset($_POST['login']))
{
    ...
    if($check_user==0)
	{
        $error_msg = 'Wrong Info Please try Again try again';
        }
    else
	{
	 ...
	 {
            setcookie('user_email', $email, time() + ( 365 * 24 * 60 * 60 *10));
            setcookie('user_pass', $pass, time() + (365 * 24 * 60 * 60 * 10));
        } 
        header('location:index.php?logged_in=Welcome, echo "$email" You Are Logged In');
    }
}
?>
<!DOCTYPE html>
